...
SCENARIO("Moguce je konstruisati translaciju", "[trans]"){
    GIVEN("Dva realna broja i bool"){
        const auto x = GENERATE_REF(-13.5, 0., 1., 1.5, 100.);
            WHEN("Postoji par brojeva"){
                const auto y = GENERATE_REF(-12., -7.1, 0., 1., 17.);
                    WHEN("Postoji i bool"){
                        const auto inv = GENERATE_REF(true, false);
...
SCENARIO("Moguce je konstruisati smicanje", "[smic]"){
    GIVEN("Dva realna broja i bool"){
        const auto x = GENERATE_REF(-13.5, -0.5, 0., 1., 1.5, 100.);
            WHEN("Postoji par brojeva"){
                const auto y = GENERATE_REF(-12., -2, 0., 1., 17.);
                    WHEN("Postoji i bool"){
                        const auto inv = GENERATE_REF(true, false);
...
SCENARIO("Moguce je konstruisati rotaciju", "[rot]"){
    GIVEN("Realan broj i bool"){
        const auto u = GENERATE_REF(-13.5, 0., 1., 1.5, 100.);
            WHEN("Postoji i bool"){
                const auto inv = GENERATE_REF(true, false);
